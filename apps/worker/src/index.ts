...
const jobAnalysisStatusEnum = pgEnum('job_analysis_status', ['PENDING', 'RUNNING', 'DONE', 'FAILED']);
const crawlStatusEnum = pgEnum('crawl_status', ['QUEUED', 'RUNNING', 'SUCCESS', 'FAILED']);
const jobStatusEnum = pgEnum('job_status', ['OPEN', 'CLOSED']);
const sourcePlatformEnum = pgEnum('source_platform', ['GREENHOUSE', 'LEVER', 'WORKDAY', 'ICIMS', 'CUSTOM', 'UNKNOWN']);
const changeTypeEnum = pgEnum('change_type', ['NEW', 'UPDATED', 'CLOSED']);
const eventTypeEnum = pgEnum('event_type', [
    'JOB_DISCOVERED', 'JOB_UPDATED', 'JOB_CLOSED',
    'CRAWL_STARTED', 'CRAWL_COMPLETED', 'CRAWL_FAILED',
    'COMPANY_ADDED', 'JOB_CLICKED',
]);
...
function generateJobSlug(companyName: string, jobId?: string, url?: string): string {
...
function detectRemote(title: string, location?: string): boolean {
    const text = `${title} ${location || ''}`.toLowerCase();
    return text.includes('remote') || text.includes('work from home') || text.includes('wfh');
}

function detectSeniority(title: string): string | undefined {
...
function detectEmploymentType(title: string): string | undefined {
    const lower = title.toLowerCase();
...
async function discoverJobLinks(careerUrl: string, platform: string, crawlRunId: string): Promise<DiscoveredJob[]> {
    await publishLog(crawlRunId, `[DISCOVERY] Starting career page crawl: ${careerUrl}`);
...
async function discoverGreenhouseJobs(careerUrl: string, crawlRunId: string): Promise<DiscoveredJob[]> {
    try {
        ...
        await publishLog(crawlRunId, `[GREENHOUSE] Fetching API: boards-api.greenhouse.io/v1/boards/${boardToken}/jobs`);
        ...
        await publishLog(crawlRunId, `[GREENHOUSE] Found ${discovered.length} jobs via API`);
        return discovered;
    } catch (error) {
        await publishLog(crawlRunId, `[GREENHOUSE] API failed, falling back to scrape`);
        return [];
    }
}

async function discoverLeverJobs(careerUrl: string, crawlRunId: string): Promise<DiscoveredJob[]> {
    try {
        ...
        await publishLog(crawlRunId, `[LEVER] Fetching API: api.lever.co/v0/postings/${company}`);

        const res = await fetch(`https://api.lever.co/v0/postings/${company}?limit=100`);
        if (!res.ok) return [];
        const data = await res.json() as any[];
        ...
        await publishLog(crawlRunId, `[LEVER] Found ${discovered.length} jobs via API`);
        return discovered;
    } catch (error) {
...
async function discoverWithPlaywright(careerUrl: string, crawlRunId: string): Promise<DiscoveredJob[]> {
    const browser = await chromium.launch({
        headless: true,
        args: ['--disable-blink-features=AutomationControlled', '--no-sandbox', '--disable-setuid-sandbox'],
    });
    ...
    try {
        ...
        await page.goto(careerUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
        await page.waitForTimeout(3000);

        // Scroll to load lazy content
        await publishLog(crawlRunId, `[PLAYWRIGHT] Scrolling page to load all content...`);
        await page.evaluate(async () => {
            await new Promise<void>((resolve) => {
                let totalHeight = 0;
                const distance = 300;
        ...
        for (const sel of loadMoreSelectors) {
            try {
                ...
                if (await btn.isVisible()) {
                    await publishLog(crawlRunId, `[PLAYWRIGHT] Clicking "Load More" button...`);
                    await btn.click();
                    await page.waitForTimeout(2000);
                }
        ...
        const discovered = await page.evaluate((baseUrl: string) => {
            const jobLinks: { title: string; url: string; location?: string; team?: string }[] = [];
            ...
            for (const link of links) {
                ...
                if (isJobLink && text && !text.match(/^(home|about|blog|contact|login|sign)/i)) {
                    seen.add(href);
                    ...
                    if (parent) {
                        const locEl = parent.querySelector('[class*="location"], [class*="city"]');
                        if (locEl) location = (locEl as HTMLElement).innerText?.trim();
                        const teamEl = parent.querySelector('[class*="team"], [class*="department"], [class*="category"]');
                        if (teamEl) team = (teamEl as HTMLElement).innerText?.trim();
                    }

                    jobLinks.push({ title: text.split('\n')[0]!.trim(), url: href, location, team });
                }
            }
        ...
        await publishLog(crawlRunId, `[PLAYWRIGHT] Found ${discovered.length} potential job links`);
        return discovered;
    } catch (error) {
        await publishLog(crawlRunId, `[PLAYWRIGHT] Error: ${error instanceof Error ? error.message : 'Unknown'}`);
        return [];
    } finally {
        await browser.close();
    }
...
async function extractJobDetail(url: string, crawlRunId: string): Promise<ExtractedData | null> {
    ...
    if (!result || (!result.title && !result.company)) {
        await publishLog(crawlRunId, `[DETAIL] Fetch incomplete, trying Playwright...`);
...
const crawlWorker = new Worker<CrawlJobData>(
    ...
    async (job: Job<CrawlJobData>) => {
        const { companyId, careerUrl, crawlRunId, platform, preferences } = job.data;
        ...
        try {
            ...
            for (let i = 0; i < discovered.length; i++) {
                ...
                try {
                    ...
                    if (existing) {
                        ...
                        const changed = existing.jobTitle !== disc.title || existing.jobLocation !== (disc.location || null);
                        await db
                            .update(jobs)
                            .set({
                                lastSeenAt: new Date(),
                        ...
                        const [newJob] = await db
                            ...
                            .values({
                                ...
                                applyUrl: disc.url,
                                jobLocation: disc.location || null,
                                jobTeam: disc.team || null,
                                employmentType: detectEmploymentType(disc.title),
                                sourcePlatform: platform as any,
                                seniority: detectSeniority(disc.title),
                                isRemote: detectRemote(disc.title, disc.location),
                        ...
                        if (!newJob) {
                            await publishLog(crawlRunId, `[WARN] Insert returned no rows for: ${disc.title}`);
                        } else {
                            // Snapshot
                            await db.insert(jobSnapshots).values({
                                jobId: newJob.id,
                                snapshotData: { title: disc.title, location: disc.location, url: disc.url },
                                changeType: 'NEW',
                            });
                            ...
                            await db.insert(events).values({
                                ...
                                entityType: 'job',
                                correlationId: crawlRunId,
                                metadata: { title: disc.title, location: disc.location, company: companyName },
                            });
                        }
                    ...
                    errorCount++;
                    await publishLog(crawlRunId, `[ERROR] Failed to process: ${disc.title} — ${error instanceof Error ? error.message : 'Unknown'}`);
                }
            }
            ...
            publisher.publish(`crawl:${crawlRunId}`, JSON.stringify({ type: 'error', error: errorMessage }));
...
const analyzeWorker = new Worker<AnalyzeJobData>(
    ...
    async (job: Job<AnalyzeJobData>) => {
        ...
        try {
            ...
            await db.update(jobAnalyses).set({ status: 'DONE', result, updatedAt: new Date() }).where(eq(jobAnalyses.id, jobAnalysisId));
            console.log(`Job analysis completed: ${jobAnalysisId}`, result);
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            console.error(`Job analysis failed: ${jobAnalysisId}`, errorMessage);
            await db.update(jobAnalyses).set({ status: 'FAILED', error: errorMessage, updatedAt: new Date() }).where(eq(jobAnalyses.id, jobAnalysisId));
            throw error;
        }
    },
...
crawlWorker.on('failed', (job, error) => console.error(`Crawl job ${job?.id} failed:`, error.message));
analyzeWorker.on('completed', (job) => console.log(`Analyze job ${job.id} completed`));
analyzeWorker.on('failed', (job, error) => console.error(`Analyze job ${job?.id} failed:`, error.message));
